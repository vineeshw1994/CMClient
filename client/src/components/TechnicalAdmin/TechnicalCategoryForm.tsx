...
function CategoryForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:4000/api/teachnical/getCategoryColumns/${id}`);
  ...
  const handleSubmit = async(e) => {
    ...
    const formattedData = Object.keys(formData).reduce((acc, columnKey) => {
      const { name } = formData[columnKey]; // Get only the key value (value is empty string by default)
      if (name) {
        acc.push({ [columnKey]: name })      }
      return acc;
    }, []);
    ...
    // Send the data to the backend
    try {
      const response = await axios.post('http://localhost:4000/api/teachnical/saveData', finalData); // Replace with your backend URL
      console.log('Data sent to backend successfully:', response.data);
      alert('Column names updated and saved successfully!');
      navigate('/technicalcategorylist')
    } catch (error) {
